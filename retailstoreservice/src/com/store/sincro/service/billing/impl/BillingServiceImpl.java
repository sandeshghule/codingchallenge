...
public class BillingServiceImpl implements BillingService{
	...
	public List<DiscountDTO> getDiscounts(String type, Double cartAmount) {
		...
		if("R".equalsIgnoreCase(type) || "REGULAR".equalsIgnoreCase(type) || "1".equalsIgnoreCase(type))
		{
			if(cartAmount > 0)
			{
		...
		else if("P".equalsIgnoreCase(type) || "PREMIUM".equalsIgnoreCase(type) || "2".equalsIgnoreCase(type))
		{
			if(cartAmount > 0)
			{
				DiscountDTO discountDTO = new DiscountDTO();
			...
			{
				DiscountDTO discountDTO = new DiscountDTO();
				populateDiscountDTO(discountDTO,cartAmount-12000D,30);
				list.add(discountDTO);
			}
		}
		return list;
